  ...
  : linearsystem< double >(name)
{
  ...
  sym  = MKL_DSS_SYMMETRIC_STRUCTURE;
  type = MKL_DSS_INDEFINITE;

  // dss: initialize
  int err;
  if (err=dss_create_(&handle, &opt))
...
dss& dss::solve()
{
  ...
  int err;
  if (/* 1: m. structure     */    (err=dss_define_structure_(&handle, &sym, &A.ia[0], &A.nnu, &A.nnu, &A.ja[0], &A.nnz))
      /* 2: m. reordering    */ || (err=dss_reorder_(&handle, &opt, NULL))
      /* 3: m. factorization */ || (err=dss_factor_real_(&handle, &type, &A.a[0]))
      /* 4: solve            */ || (err=dss_solve_real_(&handle, &opt, &m_b.a[0], &nrhs, &m_x.a[0])) )
    throw std::runtime_error(err_message(err));
  return *this;
}
